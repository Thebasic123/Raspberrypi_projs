...
SenderSocket::SenderSocket( const std::string &receiver_ip, const int receiver_port) {
...
void SenderSocket::SendPacket(const std::vector<unsigned char> &data) const {
    ...
    sendto( socket_handle_, data.data(), data.size(),
            0, const_cast<sockaddr*>(reinterpret_cast<const sockaddr*>(&receiver_addr_)),
